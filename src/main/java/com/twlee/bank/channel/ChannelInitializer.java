...
//    private final RedistTemplate redistTemplate;
    ...
    public ChannelSource sktChannelSource() {
        ...
        for (ConnectInfo connectInfo : connectInfos) {
            if (connectInfo.host().endsWith("1")) {
                pool1.add(connectInfo);
            } else {
                pool2.add(connectInfo);
            }
        }
        return new FooChannelSource(
                new ChannelConfig(Channel.A, pool1),
                new ChannelConfig(Channel.A, pool2));
    }

    private List<ConnectInfo> loadCid() {
        ConnectInfo connectInfo1 = createCid("id1", "pwd1", "127.0.0.1", "8088", "10");
        ConnectInfo connectInfo2 = createCid("id2", "pwd2", "127.0.0.1", "8089", "10");
        return List.of(connectInfo1, connectInfo2);
    }

    private ConnectInfo createCid(String id, String pwd, String host, String port, String tps) {
        return new ConnectInfo(id, pwd, host, Integer.parseInt(port), Integer.parseInt(tps));
    }
}